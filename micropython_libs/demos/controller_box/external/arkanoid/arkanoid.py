...
def main(c):
    ...
    # spi = SPI(2, baudrate=2000000, sck=Pin(pd.SPI_SCK), mosi=Pin(pd.SPI_MOSI))
    # c = controller_box.ControllerBox()
    # display = Display(spi, dc=Pin(pd.SSD1351_DC), cs=Pin(pd.SSD1351_CS), rst=Pin(pd.SSD1351_RST))
    display = c.display
    adc = c.joystick.x
    ...
    try:
        while True:
            ...
            paddle.h_position((4095 - adc.read()) // 44 + 5)
            # Handle balls
            score_points = 0
            for ball in balls:
                # Position
